...
export class SOSEscalationEntity {
  static async create(data: Omit<SOSEscalation, 'id' | 'initiatedAt'>): Promise<SOSEscalation> {
    const result = await query(
      `INSERT INTO sos_escalations (user_id, status, notes)
       VALUES ($1, $2, $3)
      ...
      [data.userId, data.status, data.notes]
    );
    return result.rows[0];
  }

  static async findById(id: string): Promise<SOSEscalation | null> {
    const result = await query(
      `SELECT id, user_id as "userId", initiated_at as "initiatedAt", status,
              manager_notified_at as "managerNotifiedAt", emergency_contacts_notified_at as "emergencyContactsNotifiedAt",
              team_notified_at as "teamNotifiedAt", resolved_at as "resolvedAt", notes
       FROM sos_escalations WHERE id = $1`,
      [id]
    );
    return result.rows[0] || null;
  }

  static async findByOrgId(orgId: string): Promise<SOSEscalation[]> {
    const result = await query(
      `SELECT s.id, s.user_id as "userId", s.initiated_at as "initiatedAt", s.status,
  ...
  static async update(id: string, data: Partial<SOSEscalation>): Promise<SOSEscalation | null> {
    ...
    const result = await query(
      `UPDATE sos_escalations SET ${updates.join(', ')} WHERE id = $${paramCount}
       RETURNING id, user_id as "userId", initiated_at as "initiatedAt", status,
                 manager_notified_at as "managerNotifiedAt", emergency_contacts_notified_at as "emergencyContactsNotifiedAt",
                 team_notified_at as "teamNotifiedAt", resolved_at as "resolvedAt", notes`,
      values
